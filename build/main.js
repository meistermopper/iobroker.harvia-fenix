						...
						for (const a of d.attr) {
							...
							switch (a.key) {
								...
								case 'stats.totalSessions.C1':
									await this.setState('totalSessions', Math.round(Number.parseInt(a.value)), true);
									break;
		...
		try {
			...
			if (p && typeof p === 'object') {
				...
				if (currentPower !== void 0) {
					currentPower = Math.round((Number.parseFloat(currentPower) / 1e3) * 100) / 100;
					await this.setState('heaterPower', currentPower, true);
				}
				if (p.totalBathingHours !== void 0) {
					await this.setState(
						'totalBathingHours',
						Math.round(Number.parseFloat(p.totalBathingHours) * 100) / 100,
						true,
					);
				}
				if (p.totalSessions !== void 0) {
					await this.setState('totalSessions', Math.round(Number.parseInt(p.totalSessions)), true);
				}
				...
				if (tTemp !== void 0) {
					await this.setState('targetTemp', Number.parseFloat(tTemp), true);
				}
	...
	async setSaunaState(stateName, value, isRetry = false) {
		...
		try {
			if (stateName === 'heatOn' || stateName === 'lightOn') {
				...
				this.log.info(`Target temperature -> ${value}\xB0C`);
